package com.cdkjframework.license.core.manger;

import com.cdkjframework.license.core.entity.LicenseExtraEntity;
import com.cdkjframework.license.core.service.AServerInfos;
import com.cdkjframework.license.core.util.DateUtils;
...
public class LicenseCustomManager extends LicenseManager {
     ...
     * 日志
     */
    private LogUtils logUtils = LogUtils.getLogger(LicenseCustomManager.class);
    ...
    protected synchronized void validate(final LicenseContent content) throws LicenseContentException {
        ...
        LicenseExtraEntity expectedCheck = (LicenseExtraEntity) content.getExtra();
        if (expectedCheck == null) {
            // 说明不需要验证服务器
            return;
        }
        //当前服务器真实的参数信息
        LicenseExtraEntity serverCheckModel = AServerInfos.getServer(null).getServerInfos();
    ...
    private Object load(String encoded) {
        ...
        try {
            ...
            try {
                ...
                logUtils.error("XMLDecoder解析XML失败", e);
            }
        }
        return null;
    ...
    private boolean checkIpAddress(List<String> expectedList, List<String> serverList) {
        ...
        if (StringUtils.isNullAndSpaceOrEmpty(expectedList)) {
            return false;
        }
        /** 如果当前服务器的IP列表空直接返回false，因为服务器不可能获取不到ip，没有的话验证个锤子 */
        if (StringUtils.isNullAndSpaceOrEmpty(serverList)) {
            return false;
        }
    ...
    private boolean checkSerial(String expectedSerial, String serverSerial) {
        if (StringUtils.isNotNullAndEmpty(expectedSerial)) {
            if (StringUtils.isNotNullAndEmpty(serverSerial)) {
