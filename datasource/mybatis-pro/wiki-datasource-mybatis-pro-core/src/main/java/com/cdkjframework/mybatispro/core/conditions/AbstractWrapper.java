...
import com.cdkjframework.mybatispro.core.annotation.OrderBy;
import com.cdkjframework.mybatispro.core.conditions.interfaces.Compare;
import com.cdkjframework.mybatispro.core.conditions.interfaces.Func;
import com.cdkjframework.mybatispro.core.conditions.interfaces.Join;
import com.cdkjframework.mybatispro.core.conditions.interfaces.Nested;
...
import com.cdkjframework.mybatispro.core.toolkit.SerializationUtils;
import com.cdkjframework.mybatispro.core.toolkit.SqlScriptUtils;
import com.cdkjframework.mybatispro.core.toolkit.sql.SqlUtils;
...
import static com.cdkjframework.mybatispro.core.enums.SqlKeyword.*;
...
public abstract class AbstractWrapper<T, R, Children extends AbstractWrapper<T, R, Children>> extends Wrapper<T>
  ...
  public <V> Children allEq(boolean condition, Map<R, V> params, boolean null2IsNull) {
    if (condition && CollectUtils.isNotEmpty(params)) {
      params.forEach((k, v) -> {
  ...
  public Children last(boolean condition, String lastSql) {
    if (condition) {
      this.lastSql.setStringValue(StringUtils.SPACE + lastSql);
    }
    return typedThis;
  }
  ...
  public Children groupBy(boolean condition, R column, R... columns) {
    return doGroupBy(condition, column, CollectUtils.toList(columns));
  }
  ...
  public Children doGroupBy(boolean condition, R column, List<R> columns) {
    return maybeDo(condition, () -> {
      String one = StringUtils.EMPTY;
      if (column != null) {
        one = columnToString(column);
      }
      if (CollectUtils.isNotEmpty(columns)) {
        one += column != null ? StringUtils.COMMA + columnsToString(columns) : columnsToString(columns);
      }
  ...
  public Children orderBy(boolean condition, boolean isAsc, R column, R... columns) {
    return doOrderBy(condition, isAsc, column, CollectUtils.toList(columns));
  }
  ...
  protected final String formatSqlMaybeWithParam(String sqlStr, Object... params) {
    if (StringUtils.isNullAndSpaceOrEmpty(sqlStr)) {
      return null;
    }
    if (ArrayUtils.isNotEmpty(params)) {
      for (int i = 0; i < params.length; ++i) {
        ...
        if (sqlStr.contains(target)) {
          ...
          if (!matcher.find()) {
            throw new GlobalRuntimeException(String.format("Please check the syntax correctness! sql not contains: \"%s\"", target));
          }
  ...
  protected ISqlSegment inExpression(Collection<?> value) {
    ...
    return () -> value.stream().map(i -> formatParam(null, i))
        .collect(joining(StringUtils.COMMA, StringUtils.LEFT_BRACKET, StringUtils.RIGHT_BRACKET));
  }
  ...
  public boolean isUseAnnotationOrderBy() {
    ...
    if (StringUtils.isNullAndSpaceOrEmpty(_sqlSegment)) {
      return true;
    }
  ...
  public String getSqlComment() {
    if (StringUtils.isNotNullAndEmpty(sqlComment.getStringValue())) {
  ...
  public String getSqlFirst() {
    if (StringUtils.isNotNullAndEmpty(sqlFirst.getStringValue())) {
  ...
  public Children setParamAlias(String paramAlias) {
    AssertUtils.isNotEmptyMessage(paramAlias, "paramAlias can not be empty!");
    AssertUtils.isEmptyMessage(paramNameValuePairs, "Please call this method before working!");
    AssertUtils.isEmptyMessage(this.paramAlias, "Please do not call the method repeatedly!");
  ...
  protected String columnsToString(R... columns) {
    return Arrays.stream(columns).map(this::columnToString).collect(joining(StringUtils.COMMA));
  }
  ...
  protected String columnsToString(List<R> columns) {
    return columns.stream().map(this::columnToString).collect(joining(StringUtils.COMMA));
  }
