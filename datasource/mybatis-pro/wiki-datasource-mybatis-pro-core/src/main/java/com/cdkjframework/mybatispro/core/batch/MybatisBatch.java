package com.cdkjframework.mybatispro.core.batch;
 ...
 * <li>autoCommit参数,在spring下使用的是{SpringManagedTransaction},控制无效,只能通过datasource控制(建议不要修改),单独使用mybatis下{@link org.apache.ibatis.transaction.jdbc.JdbcTransaction}是可用的</li>
 * <pre>
 *     Spring示例:
...
public class MybatisBatch<T> {
   ...
   * 执行批量操作
   *
   * @param statement 执行的 mapper 方法 (示例: BaseMapper.insert )
   * @return 批处理结果
   */
  ...
  public List<BatchResult> execute(boolean autoCommit, String statement, ParameterConvert<T> parameterConvert) {
    ...
    try (SqlSession sqlSession = sqlSessionFactory.openSession(ExecutorType.BATCH, autoCommit)) {
      List<List<T>> split = CollectUtils.split(dataList, batchSize);
  ...
  public static class Method<T> {
    ...
    public BatchMethod<T> insert() {
      return new BatchMethod<>(namespace + StringUtils.DOT + SqlMethod.INSERT_ONE.getMethod());
    }
    ...
    public <E> BatchMethod<E> insert(Function<E, T> function) {
      return new BatchMethod<>(namespace + StringUtils.DOT + SqlMethod.INSERT_ONE.getMethod(), function::apply);
    }

    /**
     * 更新方法 {@link BaseMapper#modifyById(Object)}
     *
     * @return 更新方法
     */
    public BatchMethod<T> modifyById() {
      return new BatchMethod<>(namespace + StringUtils.DOT + SqlMethod.UPDATE_BY_ID.getMethod(), (entity) -> {
     ...
     * @return 更新方法
     */
    public <E> BatchMethod<E> modifyById(Function<E, T> etFunction) {
      return new BatchMethod<>(namespace + StringUtils.DOT + SqlMethod.UPDATE_BY_ID.getMethod(), (parameter) -> {
        Map<String, Object> param = new HashMap<>();
    ...
    /**
     * 更新方法 {@link BaseMapper#modify(Wrapper)} (Object, Wrapper)}
     *
     * @param wrapperFunction 更新条件(不能为null)
     * @param <E>             实体
     * @return 更新方法
     */
    public <E> BatchMethod<E> modify(Function<E, Wrapper<T>> wrapperFunction) {
      return new BatchMethod<>(namespace + StringUtils.DOT + SqlMethod.UPDATE.getMethod(), (parameter) -> {
        Map<String, Object> param = new HashMap<>();
    ...
    /**
     * 更新方法 {@link BaseMapper#modify(Object, Wrapper)}
     *
     ...
     * @return 更新方法
     */
    public <E> BatchMethod<E> modify(Function<E, T> entityFunction, Function<E, Wrapper<T>> wrapperFunction) {
      return new BatchMethod<>(namespace + StringUtils.DOT + SqlMethod.UPDATE.getMethod(), (parameter) -> {
        Map<String, Object> param = new HashMap<>();
    ...
    /**
     * 删除方法 {@link BaseMapper#deleteById(Object)} or {@link BaseMapper#deleteById(Serializable)}
     *
     * @param function 参数转换
     * @param <E>      实体
     * @return 删除方法
    ...
    public <E> BatchMethod<E> deleteById(Function<E, T> function) {
      return new BatchMethod<>(namespace + StringUtils.DOT + SqlMethod.DELETE_BY_ID.getMethod(), function::apply);
    }
    ...
    public <T> BatchMethod<T> deleteById() {
      return new BatchMethod<>(namespace + StringUtils.DOT + SqlMethod.DELETE_BY_ID.getMethod());
    }

    public <E> BatchMethod<E> get(String method) {
      return new BatchMethod<>(namespace + StringUtils.DOT + method);
    }

    public <E> BatchMethod<E> get(String method, ParameterConvert<E> parameterConvert) {
      return new BatchMethod<>(namespace + StringUtils.DOT + method, parameterConvert);
    }
