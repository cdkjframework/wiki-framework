...
import com.cdkjframework.mybatispro.core.handlers.CompositeEnumTypeHandler;
import com.cdkjframework.mybatispro.core.mapper.Mapper;
import com.cdkjframework.mybatispro.core.mapper.MybatisMapperRegistry;
...
public class MybatisConfiguration extends Configuration {
  ...
  public <T> void removeMapper(Class<T> type) {
    ...
    if (mapperRegistryCache.contains(mapperType)) {
      // 清空实体表信息映射信息
      TableInfoHelper.remove(ReflectionUtils.getSuperClassGenericType(type, Mapper.class, 0));

      // 清空 Mapper 缓存信息
      this.mybatisMapperRegistry.removeMapper(type);
      this.loadedResources.remove(type.toString());
      this.loadedResources.remove(type.getName().replace(StringUtils.DOT, StringUtils.SLASH) + ".xml");
      mapperRegistryCache.remove(mapperType);

      // 清空 Mapper 方法 mappedStatement 缓存信息
      String typeKey = type.getName() + StringUtils.DOT;
    ...
    public V put(String key, V value) {
      if (containsKey(key)) {
        throw new IllegalArgumentException(name + " already contains value for " + key
            + (conflictMessageProducer == null ? StringUtils.EMPTY : conflictMessageProducer.apply(super.get(key), value)));
      }
