...
public class TableInfo implements Constants {
  ...
  public boolean havePK() {
    return StringUtils.isNotNullAndEmpty(keyColumn);
  }
  ...
  public String chooseSelect(Predicate<TableFieldInfo> predicate) {
    ...
    if (StringUtils.isNotNullAndEmpty(sqlSelect) && StringUtils.isNotNullAndEmpty(fieldsSqlSelect)) {
      return sqlSelect + COMMA + fieldsSqlSelect;
    } else if (StringUtils.isNotNullAndEmpty(fieldsSqlSelect)) {
  ...
  public String chooseSelect(Predicate<TableFieldInfo> predicate, List<String> noSelectProperty) {
    if (CollectUtils.isEmpty(noSelectProperty)) {
  ...
  public String getAllSqlWhere(boolean fistAnd, boolean ignoreLogicDelFiled, boolean withId, final String prefix) {
    ...
    if (!withId || StringUtils.isNullAndSpaceOrEmpty(keyProperty)) {
  ...
  void initResultMapIfNeed() {
    if (autoInitResultMap && null == resultMap) {
      String id = currentNamespace + DOT + MYBATIS_PRO + UNDERSCORE + entityType.getSimpleName();
      List<ResultMapping> resultMappings = new ArrayList<>();
      if (havePK()) {
        ResultMapping idMapping = new ResultMapping.Builder(configuration, keyProperty, SqlScriptUtils.getTargetColumn(keyColumn), keyType)
            .flags(Collections.singletonList(ResultFlag.ID)).build();
        resultMappings.add(idMapping);
      }
      if (CollectUtils.isNotEmpty(fieldList)) {
  ...
  void setFieldList(List<TableFieldInfo> fieldList) {
    ...
    /* 校验字段合法性 */
    AssertUtils.isTrue(logicDeleted.get() <= 1, "@TableLogic not support more than one in Class: \"%s\"", entityType.getName());
    AssertUtils.isTrue(version.get() <= 1, "@Version not support more than one in Class: \"%s\"", entityType.getName());
  }
  ...
  public Object getPropertyValue(Object entity, String property) {
    try {
      ...
      throw new GlobalRuntimeException("Error: Cannot read property in %s.  Cause:", e);
    }
  }
  ...
  public void setPropertyValue(Object entity, String property, Object... values) {
    try {
      ...
      throw new GlobalRuntimeException("Error: Cannot write property in %s.  Cause:", e);
    }
  }
