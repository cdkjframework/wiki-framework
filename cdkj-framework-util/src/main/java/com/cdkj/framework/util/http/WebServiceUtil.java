...
import com.cdkj.framework.entity.http.WebServiceEntity;
import com.cdkj.framework.util.files.XmlUtil;
...
public class WebServiceUtil<T> {
     ...
     * 日志
     */
    private static LogUtil logUtil = LogUtil.getLogger(WebServiceUtil.class);
        ...
        switch (entity.getDataFormat()) {
            ...
            default:
                if (name.equals(clazz.getName())) {
                    t = (T) JsonUtil.jsonStringToList(result, clazz);
                } else {
                    t = JsonUtil.jsonStringToBean(result, clazz);
                }
                break;
        }
        //返回结果
    ...
    public static Object[] postWebServiceObject(WebServiceEntity entity) {
        ...
        // 需要密码的情况需要加上用户名和密码
        if (!StringUtil.isNullAndSpaceOrEmpty(entity.getUserName()) && !StringUtil.isNullAndSpaceOrEmpty(entity.getPassword())) {
        ...
        try {
            ...
            if (!StringUtil.isNullAndSpaceOrEmpty(entity.getNamespace())) {
