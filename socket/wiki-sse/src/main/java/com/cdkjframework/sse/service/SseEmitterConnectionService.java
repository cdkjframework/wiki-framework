...
public class SseEmitterConnectionService {
   ...
   * 日志工具
   */
  private LogUtils logUtils = LogUtils.getLogger(SseEmitterConnectionService.class);
   ...
   */
  private final long HEARTBEAT_INTERVAL = 25L;
  ...
  public SseEmitter connect(String sessionId) {
    // 设置 15分钟超时，可根据需要调整
    long timeout = IntegerConsts.SIXTY * IntegerConsts.FIFTEEN * IntegerConsts.ONE_THOUSAND;
    ...
    // 立即发送一个欢迎消息或连接成功消息
    try {
      sseEmitter.send(SseEmitter.event().data("SSE连接建立成功", MediaType.TEXT_PLAIN).id(EmitterConstant.SUCCESS));
    } catch (IOException e) {
      disconnect(sessionId);
    }
    // 调用连接回调
